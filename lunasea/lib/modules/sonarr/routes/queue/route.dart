...
class QueueRoute extends StatefulWidget {
  const QueueRoute({Key? key}) : super(key: key);
...
class _State extends State<QueueRoute> with LunaScrollControllerMixin {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => SonarrQueueState(context.read<SonarrState>()),
  ...
  Future<void> _onRefresh(BuildContext context) async {
    await context.read<SonarrQueueState>().fetchQueue(hardCheck: true);
